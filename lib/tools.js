...
import { appendDelegatedPolicyOverrides, captureDelegatedPolicyOverrides, childSessionMeta, finalAssistantOutput, resolveChildAgentOptions, resolveChildDepth, } from '@deepseek-ai/dsh-subagent';
/**
 ...
 */
function toStopReason(reason, cancelled) {
    switch (reason?.kind) {
        case 'completed':
            return 'completed';
        case 'max-tokens':
 ...
 */
function wireReasoningEffort(childCtx, effort) {
 ...
 */
async function runWithPreset(ctx, profile, parent, task, signal) {
    const childDepth = resolveChildDepth(parent, 3);
    const meta = childSessionMeta(parent, childDepth, 0);
    const agentOptions = resolveChildAgentOptions(parent, profile.model !== '' ? splitModel(profile.model) : undefined, childDepth);
    ...
    const createOptions = {
        ...
        setup: async (childCtx) => {
            ...
            if (shouldWireEffort)
                wireReasoningEffort(childCtx, effort);
        },
    };
    ...
    try {
        ...
        const result = { output, stopReason };
 ...
 */
async function runInherited(ctx, profile, parent, task, signal) {
